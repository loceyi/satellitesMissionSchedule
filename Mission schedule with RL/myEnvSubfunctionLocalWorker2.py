...
import globalVariableLocalWorker2
#self.state = np.array([Storage,TaskNumber,label])

def get_env_feedback(S, A):
    done=0
    # satStateTable=globalVariableLocal.get_value_satState()
    globalVariableLocalWorker2.taskListMove(S[1]) #更新完global值后要取出来
    taskList=globalVariableLocalWorker2.get_value_taskList()
    TaskTotal=globalVariableLocalWorker2.get_value_TaskTotal() #返回整个task字典变量
    # This is how agent will interact with the environment
    Tasknum = S[1]
    TaskRequirement=globalVariableLocalWorker2.get_value_Task(str(Tasknum)).copy()
    # S[2]=taskList[0]
    # RemainingTime = S[1]

    RemainingTimeTotal=globalVariableLocalWorker2.get_value_RemainingTimeTotal()
    ...
    if A == 1: #Accept=1
        ...
        for i in range(0,len(taskList)):
            ...
            if taskList[0] == 0:
                ...
                if S[0] < TaskTotal[str(taskList[0])][2] or Counter == 0:
                    ...
                    globalVariableLocalWorker2.taskListPop()
                    taskList = globalVariableLocalWorker2.get_value_taskList()
                    S[1] = taskList[0]
        ...
        if diff == len(RemainingTimeTotal):
            ...
            globalVariableLocalWorker2.updateRemainTimeTotal(RemainingTime)
